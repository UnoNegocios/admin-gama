  ...
  <v-container class="pa-0">
        <v-card class="elevation-0 pa-6">
            <v-row class="ma-0 mb-3">
                <v-card-title>Editar Campaña</v-card-title>
                <v-spacer/>
                <v-switch
            ...
            <v-card-actions class="px-0">
                ...
                <v-btn :disabled="gris" @click="save()" class="elevation-0 px-4" color="primary"><strong>Guardar</strong></v-btn>
            </v-card-actions>
        </v-card>
...
export default {
    ...
    methods:{
        ...
        save(){
            axios.patch(process.env.VUE_APP_BACKEND_ROUTE + "api/v1/campaigns/" + this.campaign.id, this.campaign).then(response=>{
                this.close()
            }).catch(error=>{
                this.snackbar = {
